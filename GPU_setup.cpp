...
Notes:
	- Every function must be implemented (even just a dummy implementation is fine).
		SDL_gpu does not do nullptr checks on the renderer implementation function pointers because it is better to have to
		copy and paste a bunch of dummy functions than to force every good renderer to do extra nullptr checks.
	- When SDL_gpu calls one of these functions internally, 'renderer' and 'renderer->current_context_target' will never be nullptr.
...
GPU_Target* CreateTargetFromWindow(GPU_Renderer* renderer, Uint32 windowID, GPU_Target* target)
{
	...
	if (target == nullptr)
	{
		...
		target->refcount = 1;
		target->is_alias = 0;
		target->data = nullptr;  // Allocate a data structure as needed for other render target data
		...
		target->context->data = nullptr;  // Allocate a data structure as needed for other context data
	...
	target->viewport = GPU_MakeRect(0.0f, 0.0f, static_cast< float >( target->context->window_w ), static_cast< float >( target->context->window_h ));
...
GPU_bool SetActiveTarget(GPU_Renderer* renderer, GPU_Target* target)
{
	GPU_Log(" %s (dummy)\n", __func__);
	if (renderer->current_context_target == nullptr)
		return GPU_FALSE;
...
GPU_bool AddDepthBuffer(GPU_Renderer* renderer, GPU_Target* target)
{
	GPU_Log(" %s (dummy)\n", __func__);
	return GPU_TRUE;
}

GPU_bool SetWindowResolution(GPU_Renderer* renderer, Uint16 w, Uint16 h)
{
...
GPU_bool SetFullscreen(GPU_Renderer* renderer, GPU_bool enable_fullscreen, GPU_bool use_desktop_resolution)
{
	GPU_Target* target = renderer->current_context_target;
...
GPU_Image* CreateImage(GPU_Renderer* renderer, Uint16 w, Uint16 h, GPU_FormatEnum format)
{
	...
	result->data = nullptr;  // Allocate a data structure as needed for other image data
	result->is_alias = 0;
...
GPU_Image* CreateImageUsingTexture(GPU_Renderer* renderer, GPU_TextureHandle handle, GPU_bool take_ownership)
{
	GPU_Log(" %s (dummy)\n", __func__);
...
GPU_bool SaveImage(GPU_Renderer* renderer, GPU_Image* image, const char* filename, GPU_FileFormatEnum format)
{
	SDL_Surface* surface;
...
void UpdateImage(GPU_Renderer* renderer, GPU_Image* image, const GPU_Rect* image_rect, SDL_Surface* surface, const GPU_Rect* surface_rect)
{
	GPU_Log(" %s (dummy)\n", __func__);
}
...
GPU_bool ReplaceImage(GPU_Renderer* renderer, GPU_Image* image, SDL_Surface* surface, const GPU_Rect* surface_rect)
{
	GPU_Log(" %s (dummy)\n", __func__);
	return 0;
}
...
GPU_Target* GetTarget(GPU_Renderer* renderer, GPU_Image* image)
{
	GPU_Target* result;
	...
	result->refcount = 0;
	result->data = nullptr;  // Allocate a data structure as needed for other render target data
...
void FreeTarget(GPU_Renderer* renderer, GPU_Target* target)
{
	...
	if (target->context != nullptr && target->context->failed)
	{
	...
	if (!target->is_alias && target->image != nullptr)
		target->image->target = nullptr;  // Remove reference to this object
...
{
	GPU_Blit( image, src_rect, target, x, y );
	GPU_Log(" %s (dummy)\n", __func__);
}
...
{
	GPU_BlitTransformX( image, src_rect, target, x, y, pivot_x, pivot_y, degrees, scaleX, scaleY );
	GPU_Log(" %s (dummy)\n", __func__);
}


void PrimitiveBatchV(GPU_Renderer* renderer, GPU_Image* image, GPU_Target* target, GPU_PrimitiveEnum primitive_type, unsigned short num_vertices, void* values, unsigned int num_indices, unsigned short* indices, GPU_BatchFlagEnum flags)
{
	GPU_Log(" %s (dummy)\n", __func__);
}
...
GPU_TextureHandle GetTextureHandle(GPU_Renderer* renderer, GPU_Image* image)
{
	GPU_Log(" %s (dummy)\n", __func__);
	return 0;
}
...
Uint32 CompileShader_RW(GPU_Renderer* renderer, GPU_ShaderEnum shader_type, SDL_RWops* shader_source, GPU_bool free_rwops)
{
	GPU_Log(" %s (dummy)\n", __func__);
...
GPU_bool LinkShaderProgram(GPU_Renderer* renderer, Uint32 program_object)
{
	GPU_Log(" %s (dummy)\n", __func__);
...
void SetUniformMatrixfv(GPU_Renderer* renderer, int location, int num_matrices, int num_rows, int num_columns, GPU_bool transpose, float* values)
{
	GPU_Log(" %s (dummy)\n", __func__);
}
